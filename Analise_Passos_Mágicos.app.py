...
tab0, tab1, tab2, tab3, tab4 = st.tabs(["Conheça a Passos Mágicos","Resultados Educacionais", "Análises Externas", "Previsões", "Análise SocioEconômica"])
...
with tab3:
    ...
    with tab4:
        ...
        df=pd.read_excel('https://raw.githubusercontent.com/lucianesantos/DataThon/main/PSE2020_domicilios_1.xlsx')
        df3 = pd.read_excel('https://raw.githubusercontent.com/lucianesantos/DataThon/main/PSE2020_domicilios_2.xlsx')

        ###Entrevistados por polo
        ...
        #grafico polos 
        cores = [ '#FF5931', '#FBBA00' ,'#FFD401',  '#FF2E34' ]
        


        #grafico
        grafico_polo = df_polos.plot(grid='gray',y='total_por_polo', x='polos', kind='bar', color=cores, title='Entrevistados por polo',fontsize=16)
        fig3 = grafico_polo.get_figure()
        fig3.set_size_inches(16, 6)  
 

        grafico_polo.set_title('Famílias Entrevistadas por Polo\n', fontsize=26)
        plt.xticks(rotation=45)
        plt.xlabel('')

        st.pyplot(fig3)
        st.markdown('')
        st.markdown('<p style="text-align: justify;font-size:18px;">Em 2020 foi feito uma pesquisa com as famílias que fazem parte da ONG Passos Mágicos, e mediante esses dados disponibilizados foi possível deduzir qual é a situação dessas famílias. </p>', unsafe_allow_html=True)
        st.markdown('<p style="text-align: justify;font-size:18px;">A pesquisa foi feita em 4 unidades da Passos, sendo elas a unidade do Centro, do bairro do Filipinho, Granjinha e Cipó. Percebendo que a maior quantidade de pessoas entrevistadas está na unidade do Centro de Embu-Guaçu. </p>', unsafe_allow_html=True)
        st.markdown('')
 ...
 ###mapa
        ...
        # mapa = fl.Map(centro, zoom_start=14)
       ...
       #  mapa_html = mapa.get_root().render()
        ...
        # Incorporar o mapa usando IFrame
       #  st.components.v1.html(
         #    f'<iframe srcdoc="{mapa_html}" width="100%" height="600px"></iframe>',
       #  height=600
       # )
        ...
        #grafico desempregado 
        contagem_desempregado = df3['total_desempregados'].value_counts().reset_index()
        contagem_desempregado.columns = ['total_desempregados', 'contagem']
        
        grafico_desempregado = contagem_desempregado.plot(grid='gray',x='total_desempregados', y='contagem', kind='bar', color=cores, title='Total Desempregado',fontsize=16)
        fig3 = grafico_desempregado.get_figure()
        fig3.set_size_inches(16, 6)  
        grafico_desempregado.set_title('Total Desempregados\n', fontsize=26)
        plt.xticks(rotation=45)
        plt.xlabel('')
        st.pyplot(fig3)
        st.markdown('')
        st.markdown('<p style="text-align: justify;font-size:18px;">Observando os dados sobre as pessoas que estão trabalhando dentro da moradia dessas famílias, podemos ver que 87,8% das famílias tem uma ou mais pessoas trabalhando dentro de casa. </p>', unsafe_allow_html=True)
        st.markdown('')
        fig = px.pie(df, values=contagem_desempregado['contagem'], names=contagem_desempregado['total_desempregados'], color_discrete_sequence=cores)
        st.plotly_chart(fig)


        #grafico trabalho informal
        contagem_trab_informal = df3['total_trab_informal'].value_counts().reset_index()
        contagem_trab_informal.columns = ['total_trab_informal', 'contagem']
        
        grafico_trab_informal = contagem_trab_informal.plot(grid='gray',x='total_trab_informal', y='contagem', kind='bar', color=cores, title='Total Desempregado',fontsize=16)
        fig3 = grafico_trab_informal.get_figure()
        fig3.set_size_inches(16, 6)  
        grafico_trab_informal.set_title('Total Trabalho Informal\n', fontsize=26)
        plt.xticks(rotation=45)
        plt.xlabel('')
        st.pyplot(fig3)
        st.markdown('')
        st.markdown('')
        st.markdown('')
       #grafico trabalho formmal

        contagem_trab_formal = df3['total_trab_formal'].value_counts().reset_index()
        contagem_trab_formal.columns = ['total_trab_formal', 'contagem']
        
        grafico_trab_formal = contagem_trab_formal.plot(grid='gray',x='total_trab_formal', y='contagem', kind='bar', color=cores, title='Total Desempregado',fontsize=16)
        fig3 = grafico_trab_formal.get_figure()
        fig3.set_size_inches(16, 6)  
        grafico_trab_formal.set_title('Total Trabalho Formal\n', fontsize=26)
        plt.xticks(rotation=45)
        plt.xlabel('')
        st.pyplot(fig3)
        st.markdown('')
        st.markdown('<p style="text-align: justify;font-size:18px;">Sendo que 56,7% das pessoas que trabalham tem o tipo de trabalho sendo formal, e 56,3% são de pessoas que trabalham informalmente, isso levando em consideração todas as pessoas que responderam a pesquisa e também que algumas pessoas exercem um trabalho forma e um informal ao mesmo tempo. </p>', unsafe_allow_html=True)
        st.markdown('')

        fig = px.pie(df, values=contagem_trab_informal['contagem'], names=contagem_trab_informal['total_trab_informal'], color_discrete_sequence=cores)
        st.plotly_chart(fig)

        fig = px.pie(df, values=contagem_trab_formal['contagem'], names=contagem_trab_formal['total_trab_formal'], color_discrete_sequence=cores)
        st.plotly_chart(fig)


        #grafico bolsa familia 

        contagem_bolsa_familia = df3['total_bolsa_familia'].value_counts().reset_index()
        contagem_bolsa_familia.columns = ['total_bolsa_familia', 'contagem']
        
        grafico_bolsa_familia = contagem_bolsa_familia.plot(grid='gray',x='total_bolsa_familia', y='contagem', kind='bar', color=cores, title='Total Desempregado',fontsize=16)
        fig3 = grafico_bolsa_familia.get_figure()
        fig3.set_size_inches(16, 6)  
        grafico_bolsa_familia.set_title('Total Bolsa Familia', fontsize=26)
        plt.xticks(rotation=45)
        plt.xlabel('')
        st.pyplot(fig3)
        st.markdown(' ')

        #grafico outros programas

        contagem_outros_programas = df3['total_indi_outros_programas'].value_counts().reset_index()
        contagem_outros_programas.columns = ['total_indi_outros_programas', 'contagem']
        
        grafico_outros_programas = contagem_outros_programas.plot(grid='gray',x='total_indi_outros_programas', y='contagem', kind='bar', color=cores, title='Total Desempregado',fontsize=16)
        fig3 = grafico_outros_programas.get_figure()
        fig3.set_size_inches(16, 6)  
        grafico_outros_programas.set_title('Total Outros Programas\n', fontsize=26)
        plt.xticks(rotation=45)
        plt.xlabel('')
        st.pyplot(fig3) 
        st.markdown('<p style="text-align: justify;font-size:18px;">Para auxiliar nos custos mensais das familias, algumas delas são cadastradas em programas sociais. Como podemos ver nos graficos abaixo, somente 28,7% das familias estão no bolsa familia e 7,8% estão em outros projetos sociais.. </p>', unsafe_allow_html=True)
        st.markdown(' ')

#faixa_total_renda

        contagem_faixa_total_renda = df3['faixa_total_renda'].value_counts().reset_index()
        contagem_faixa_total_renda.columns = ['faixa_total_renda', 'contagem']
        
        grafico_total_renda = contagem_faixa_total_renda.plot(grid='gray',x='faixa_total_renda', y='contagem', kind='bar', color=cores, title='Total Desempregado',fontsize=16)
        fig3 = grafico_total_renda.get_figure()
        fig3.set_size_inches(16, 6)  
        grafico_total_renda.set_title('Faixas de Renda\n', fontsize=26)
        plt.xticks(rotation=75)
        plt.xlabel('')
        st.pyplot(fig3) 
        st.markdown('')
        st.markdown('<p style="text-align: justify;font-size:18px;">Indo um pouco mais a fundo no assunto financeiro das familias podemos ver que se juntarmos a quantidade de pessosas que recebem um valor até R$2090,00 de renda mensal é equivale a 43,7% das familias que fazem parte do projeto. Levando em cosideração que é 4 pessoas a média de morador por residencia, mostrada na pesquisa, consegumos ver o quão baixo é a renda nessas familias e o quanto é importante a passos na vida delas para oferecer mais oportunidade e mudanças. </p>', unsafe_allow_html=True)
